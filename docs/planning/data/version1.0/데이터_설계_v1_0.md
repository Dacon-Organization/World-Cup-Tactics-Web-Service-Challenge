...
| 엔티티 | 물리 위치 | 왜 분리했나 |
|---|---|---|
| **TEAM / PLAYER** | `players.json` | 선수는 팀에 소속되지만, 상대 팀 선수도 리플레이 이벤트의 행위자(남아공전 63' 득점자 등)이므로 4팀 전체를 동일 스키마로 관리. `displayName`은 가공명만 — **실명을 담을 필드 자체가 없어** 스키마 수준에서 P7 위반을 차단 |
